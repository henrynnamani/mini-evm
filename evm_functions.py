...
def add(evm):
    ...
    evm.pc += 1
    evm.gas_desc(THREE_GAS)

def mul(evm):
    a, b = evm.stack.pop(), evm.stack.pop()
    evm.stack.push((a * b) & UINT_256_MAX)
    evm.pc += 1
    evm.gas_desc(FIVE_GAS)
...
def exp(evm):
    ...
    evm.pc += 1
    evm.gas_desc(10 + (50 * size_in_bytes(exponent)))
...
def balance(evm):
    ...
    evm.pc += 1
    evm.gas_desc(100 if warm else 2600)
...
def calldatacopy(evm):
    ...
    evm.gas_desc(THREE_GAS + dynamic_gas)
    evm.pc += 1
...
def extcodecopy(evm):
    ...
    evm.gas_desc(dynamic_gas + address_access_cost)
    evm.pc += 1
...
def sload(evm):
    ...
    evm.gas_desc(100 if warm else 2100)  # warm=cheap, cold=expensive
    evm.pc += 1
...
def sstore(evm):
    ...
    evm.gas_desc(base_dynamic_gas + access_cost)
    evm.pc += 1
...
def _log(evm, topic_count):
    ...
    evm.pc += 1
    evm.gas_desc(calc_log_gas(topic_count, size, memory_expansion_cost))
